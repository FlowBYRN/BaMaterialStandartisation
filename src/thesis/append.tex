\chapter{Anhang}
\label{a:append}
\section*{Abbildungen}

\begin{figure}[H]
	\centering
	\includegraphics[width=1\linewidth]{ifc-manager}
	\caption{Oberfläche des \ac{ifc} Manager}
	\label{fig:ifc-manager}
\end{figure}

\begin{figure}[H]
	\centering
	\includegraphics[width=1\linewidth]{material-layer-set}
	\source{\cite{ifc_layerset_graph}}
	\caption{\ac{ifc}: Material Layer Set Association}
	\label{fig:layer-set}
\end{figure}

\begin{sidewaysfigure}
	\centering
	\includegraphics[width=\textheight]{material-categories}
	\caption{Überkategorien für die Klassifizierung von Materialien mit Beispielen}
	\label{fig:material-categories}
\end{sidewaysfigure}

\begin{figure}[H]
	\centering
	\includegraphics[width=1\linewidth]{material-profile-set}
	\source{\cite{ifc_profileset_graph}}
	\caption{\ac{ifc}: Material Profile Set Association}
	\label{fig:profile-set}
\end{figure}

\begin{figure}[H]
	\centering
	\includegraphics[width=1\linewidth]{material-constituent-set}
	\source{\cite{ifc_constituentset_graph}}
	\caption{\ac{ifc}: Material Constituent Set Association}
	\label{fig:constituent-set}
\end{figure}

\begin{figure}[H]
	\centering
	\includegraphics[width=1\linewidth]{cost-structure-example}
	\caption{Beispiel einer Kostengliederung in der ORCA AVA}
	\label{fig:cost-structure}
\end{figure}

\begin{figure}[H]
	\centering
	\includegraphics[width=1\linewidth]{verteilungsdiagramm}
	\caption{Verteilungsdiagramm der Architektur}
	\label{fig:distribution-diagramm}
\end{figure}

\begin{figure}[H]
	\centering
	\includegraphics[width=1\linewidth]{function-import}
	\caption{Flussdiagramm Import der Material-Kostengliederung}
	\label{fig:func-import}
\end{figure}

\begin{figure}[H]
	\centering
	\includegraphics[width=1\linewidth]{function-takeover}
	\caption{Flussdiagramm Übernahme von Mengen aus dem \ac{ifc} Manager}
	\label{fig:func-takeover}
\end{figure}

\begin{figure}[H]
	\centering
	\includegraphics[width=1\linewidth]{function-rating}
	\caption{Flussdiagramm Bewertung des Kostengliederungs-Import}
	\label{fig:func-rating}
\end{figure}

\begin{figure}[H]
	\centering
	\includegraphics[width=1\linewidth]{material-classifier-gui}
	\caption{Oberfläche des Klassifizierungs-Tools für Materialien}
	\label{fig:classification-service}
\end{figure}

\begin{figure}[H]
	\centering
	\includegraphics[width=\textwidth]{mldotnet-workflow}
	\source{\cite{mlnet_doc2022}}
	\caption{Code Workflow von \textit{ML.NET}}
	\label{fig:mlnet-workflow}
\end{figure}


\newpage
\section*{Listings}
\label{c:listings}

\lstinputlisting[language={[Sharp]C}, caption={Implementierung der Methode \code{GenerateMaterialCostStructureAsync} der Klasse \code{MateialService}}, style={csharp}, label={lst:materialservice}]{MaterialService.cs}

%\lstinputlisting[language={[Sharp]C}, caption={Python-Interopt für das Preprocessing}, style={csharp}, label={lst:python-preprocess}]{PythonPreprocess.cs}

\lstinputlisting[language={python}, caption={Pythonskript \textit{preprocess.py}, welches das Preprocessing der Materialstrukturierung ausführt.}, label={Script}, label={lst:preprocess}]{preprocess.py}

\newpage
\section*{Tabellen}
\label{c:tables}
\input{tables/confusion-matrix}
\input{tables/evaluation-examples}
